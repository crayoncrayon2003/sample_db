...
def aggregate_search():
    ...
    print("=== AGGREGATE WINDOW TEST ===")

    client = connect()

    print("aggregate Alice by 1 hour window")

    # aggregateWindow で 1 時間ごとにまとめ、平均値を計算する (ダウンサンプリング)
    query = """
    from(bucket: "{bucket}")
    ...
    for table in client.query_api().query(query, org=org):
        for record in table.records:
            print("Data row =", record.get_time(), record.values.get("sensor"), "avg =", round(record.get_value(), 2))
