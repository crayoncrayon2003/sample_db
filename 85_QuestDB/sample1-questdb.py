import psycopg2

host = "localhost"
port = 8812
user = "admin"
password = "quest"
dbname = "qdb"

# (時刻, センサー名, 計測値) の時系列データ
readings = [
    ("2026-01-01T00:00:00.000000", "Alice", 10.0),
    ("2026-01-01T00:15:00.000000", "Alice", 12.0),
    ("2026-01-01T00:30:00.000000", "Alice", 11.0),
    ("2026-01-01T01:00:00.000000", "Alice", 20.0),
    ("2026-01-01T01:30:00.000000", "Alice", 22.0),
    ("2026-01-01T00:00:00.000000", "Bob", 5.0),
    ("2026-01-01T00:20:00.000000", "Bob", 7.0),
    ("2026-01-01T01:10:00.000000", "Bob", 9.0),
]


def connect():
    conn = psycopg2.connect(
        host=host, port=port, user=user, password=password, dbname=dbname
    )
    # QuestDB は PostgreSQL ワイヤ互換だが、autocommit で使うのが無難
    conn.autocommit = True
    return conn


def timeseries_test():

    print("=== TIMESERIES TEST ===")

    conn = connect()
    cursor = conn.cursor()

    print("create table")

    cursor.execute("DROP TABLE IF EXISTS metrics;")
    # ts を指定タイムスタンプ (designated timestamp) にした時系列テーブル
    cursor.execute("""
        CREATE TABLE metrics (
            ts     TIMESTAMP,
            sensor SYMBOL,
            value  DOUBLE
        ) TIMESTAMP(ts) PARTITION BY DAY;
    """)

    print("insert data")

    for t, sensor, value in readings:
        cursor.execute(
            "INSERT INTO metrics VALUES (%s, %s, %s);",
            (t, sensor, value),
        )

    print("query data")

    cursor.execute("SELECT ts, sensor, value FROM metrics ORDER BY ts, sensor;")
    for row in cursor.fetchall():
        print("Data row =", row[0], row[1], row[2])

    cursor.close()
    conn.close()


def filtered_query():

    print("=== FILTERED QUERY TEST ===")

    conn = connect()
    cursor = conn.cursor()

    print("query data with filter (sensor = Alice)")

    cursor.execute(
        "SELECT ts, sensor, value FROM metrics WHERE sensor = %s ORDER BY ts;",
        ("Alice",),
    )
    for row in cursor.fetchall():
        print("Data row =", row[0], row[1], row[2])

    cursor.close()
    conn.close()


if __name__ == "__main__":

    timeseries_test()

    filtered_query()
